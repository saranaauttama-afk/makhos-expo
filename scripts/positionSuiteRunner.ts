...
import { ExtensionFeatureFlags, fixedDepthSearch, fixedNodeSearch, MoveOrderingFeatureFlags,
  SearchFeatureFlags, SearchMeasurementOptions } from '../src/coreClaude/search/alphabeta';
...
export async function runPositionSuite(mode: 'nodes' | 'depth' = 'nodes', budget = CANONICAL_POSITION_NODE_BUDGET,
  extensionOverrides?: Partial<ExtensionFeatureFlags>, options: {
    searchFeatures?: Partial<SearchFeatureFlags>;
    moveOrdering?: Partial<MoveOrderingFeatureFlags>;
    measurement?: SearchMeasurementOptions;
  } = {}) {
  const rows: PositionRunRow[] = [];
  for (const c of POSITION_SUITE) {
    const result = mode === 'nodes'
      ? await fixedNodeSearch(c.position, budget, new TT(), c.historyHashes, 64, undefined,
        options.searchFeatures, extensionOverrides, options.moveOrdering, options.measurement)
      : await fixedDepthSearch(c.position, budget, new TT(), c.historyHashes, undefined,
        options.searchFeatures, extensionOverrides, options.moveOrdering, options.measurement);
