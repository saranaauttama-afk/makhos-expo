...
import {
  PHASE3G_PRUNING_START_SUITE,
  PHASE3G_PRUNING_V1_FINGERPRINT,
} from './tournamentStartSuite';

const CONFIG = 'config/phase3g-pruning-v1.json';
type Candidate = Exclude<PruningProfile, 'current'>;
type Config = {
...
const metrics = (totals: EngineTotals) => ({
  ...
  nps: totals.nps,
  moves: totals.moves,
  pruningStats: totals.pruningStats,
});
...
function validate() {
  if (config.suite !== PHASE3G_PRUNING_START_SUITE.version ||
      config.seed !== PHASE3G_PRUNING_START_SUITE.seed ||
      config.fingerprint !== PHASE3G_PRUNING_V1_FINGERPRINT) {
    throw new Error('frozen Phase 3G corpus identity mismatch');
  }
  if (JSON.stringify(config.baselineSearch) !== JSON.stringify(DEFAULT_SEARCH_FEATURES) ||
      ...
      JSON.stringify(config.baselineMoveOrdering) !== JSON.stringify(DEFAULT_MOVE_ORDERING_FEATURES) || config.baselineLmrProfile !== 'current' || config.baselinePruningProfile !== 'current') {
...
export function makePhase3gEnginePair(feature: Candidate, search: SearchControl): [EngineConfig, EngineConfig] {
  const instrument = search.mode === 'nodes';
  const baselineMeasurement = { lmrProfile:'current', pruningProfile:'current', ...(instrument?{collectPruningStats:true}:{}) } as const;
  const candidateMeasurement = { lmrProfile:'current', pruningProfile:feature, ...(instrument?{collectPruningStats:true}:{}) } as const;
  return [
    {
      id: 'baseline',
    ...
    {
      id: `profile-${feature}`,
      name: `Teacher v1 pruning ${feature}`,
      search,
      featureOverrides: config.baselineSearch,
      extensionOverrides: config.baselineExtensions,
      moveOrderingOverrides: config.baselineMoveOrdering,
...
async function run(feature: Candidate, control: SearchControl, offset: number, pairs: number,
  ...
  const [baseline, candidate] = makePhase3gEnginePair(feature, control);
  const result = await runTournament(baseline, candidate, PHASE3G_PRUNING_START_SUITE, {
    startOffset: offset,
    startLimit: pairs,
    maxPlies,
...
async function main() {
  ...
  const output = arg('output') ?? `.tmp/phase3g-${stage}`;
  ...
  if (stage === 'screening') {
    ...
    writeFileSync(`${output}/summary.json`, JSON.stringify({ schemaVersion: 'makhos-phase3g-screening-v1',
  ...
  writeFileSync(`${output}/summary.json`, JSON.stringify({ schemaVersion: 'makhos-phase3g-confirmation-v1',
