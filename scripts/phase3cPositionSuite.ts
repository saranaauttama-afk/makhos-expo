...
(async () => {
  const baselineRows = await runPositionSuite('nodes', 5_000);
  const candidateRows = await runPositionSuite('nodes', 5_000, { smallEndgame: false });
  // Selection is already frozen. Only now create evaluation artifacts containing holdout rows.
  ...
  writeFileSync('.tmp/phase3c/position-suite/comparison.json', JSON.stringify({ baseline, candidate }, null, 2) + '\n');
