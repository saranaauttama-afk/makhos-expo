...
import { DEFAULT_SEARCH_FEATURES, fixedDepthSearch, fixedNodeSearch, iterativeDeepening,
  AspirationStats, ExtensionFeatureFlags, ExtensionStats, LmrStats, MoveOrderingFeatureFlags, MoveOrderingStats, PruningStats, SearchMeasurementOptions, resetSearchHeuristicsForMeasurement, SearchFeatureFlags, SearchResult } from '../src/coreClaude/search/alphabeta';
...
export interface MoveMetric { ply: number; engineId: string; side: Side; move: Move; nodes: number; qnodes: number; depth: number; elapsedMs: number; nps: number; extensionStats?: ExtensionStats; moveOrderingStats?: MoveOrderingStats; lmrStats?:LmrStats; pruningStats?:PruningStats; aspirationStats?:AspirationStats }
...
export interface EngineTotals { wins: number; draws: number; nodes: number; qnodes: number; elapsedMs: number; moves: number; averageDepth: number; nps: number; extensionStats?: ExtensionStats; moveOrderingStats?: MoveOrderingStats; lmrStats?:LmrStats; pruningStats?:PruningStats; aspirationStats?:AspirationStats }
...
export async function playTournamentGame(start: TournamentStart, pairId: string, gameInPair: 1|2,
  ...
  for (let ply = 0; ply < maxPlies; ply++) {
    ...
    moveMetrics.push({ ply, engineId: engine.id, side: pos.side, move: legalMove, nodes: found.nodes, qnodes: found.qnodes,
      depth: found.depth, elapsedMs: found.elapsedMs, nps: found.elapsedMs > 0 ? totalNodes * 1000 / found.elapsedMs : 0, extensionStats:found.extensionStats, moveOrderingStats:found.moveOrderingStats, lmrStats:found.lmrStats, pruningStats:found.pruningStats, aspirationStats:found.aspirationStats });
...
function totals(id: string, games: GameRecord[]): EngineTotals {
  const ms=games.flatMap(g=>g.moveMetrics).filter(m=>m.engineId===id);
  const nodes=ms.reduce((s,m)=>s+m.nodes,0),qnodes=ms.reduce((s,m)=>s+m.qnodes,0),elapsedMs=ms.reduce((s,m)=>s+m.elapsedMs,0);
  const stats={} as ExtensionStats,ordering={} as MoveOrderingStats;
  let lmr:LmrStats|undefined, pruning:PruningStats|undefined, aspiration:AspirationStats|undefined;
  for(const m of ms){
    for(const [k,v] of Object.entries(m.extensionStats??{})){const x=stats[k as keyof ExtensionStats]??={triggers:0,addedDepth:0,rootTriggers:0,rootAddedDepth:0,interiorTriggers:0,interiorAddedDepth:0};for(const field of Object.keys(x) as (keyof typeof x)[])x[field]+=v[field];}
    for(const [k,v] of Object.entries(m.moveOrderingStats??{})){const x=ordering[k as keyof MoveOrderingStats]??={scoreHits:0,firstAfterOrdering:0,updates:0};for(const field of Object.keys(x) as (keyof typeof x)[])x[field]+=v[field];}
    if(m.lmrStats){lmr??={eligibleMoves:0,reducedMoves:0,totalReducedPlies:0,reductionHistogram:[0,0,0,0],fullDepthResearches:0,root:{eligibleMoves:0,reducedMoves:0,totalReducedPlies:0,reductionHistogram:[0,0,0,0],fullDepthResearches:0},interior:{eligibleMoves:0,reducedMoves:0,totalReducedPlies:0,reductionHistogram:[0,0,0,0],fullDepthResearches:0}};for(const part of [undefined,'root','interior'] as const){const to=part?lmr[part]:lmr,from=part?m.lmrStats[part]:m.lmrStats;to.eligibleMoves+=from.eligibleMoves;to.reducedMoves+=from.reducedMoves;to.totalReducedPlies+=from.totalReducedPlies;to.fullDepthResearches+=from.fullDepthResearches;for(let i=0;i<4;i++)to.reductionHistogram[i]+=from.reductionHistogram[i];}}
    if(m.pruningStats){pruning??=makeEmptyPruningStats();for(const family of Object.keys(pruning) as (keyof PruningStats)[]){const to=pruning[family] as any,from=m.pruningStats[family] as any;for(const field of ['eligibleNodes','attempted','cutoffs','nullSearches','capturesTried','successfulCutoffs','breakEvents','estimatedMovesSkipped'])if(from[field]!==undefined)to[field]+=from[field];for(let i=0;i<65;i++)to.depthHistogram[i]+=from.depthHistogram[i];}}
    if(m.aspirationStats){aspiration??=makeEmptyAspirationStats();const from=m.aspirationStats;for(const field of ['completedIterations','aspirationIterations','failLow','failHigh','retries','failedAttemptNodes','failedAttemptQnodes'] as const)aspiration[field]+=from[field];aspiration.maxRetriesForOneDepth=Math.max(aspiration.maxRetriesForOneDepth,from.maxRetriesForOneDepth);for(let i=0;i<from.retryHistogram.length;i++)aspiration.retryHistogram[i]=(aspiration.retryHistogram[i]??0)+(from.retryHistogram[i]??0);for(const field of ['initialWindowWidthHistogram','finalWindowWidthHistogram'] as const)for(const [key,value] of Object.entries(from[field]))aspiration[field][key]=(aspiration[field][key]??0)+value;}
  }
  return {wins:games.filter(g=>g.winnerEngineId===id).length,draws:games.filter(g=>g.result==='draw').length,nodes,qnodes,elapsedMs,moves:ms.length,averageDepth:ms.length?ms.reduce((s,m)=>s+m.depth,0)/ms.length:0,nps:elapsedMs?(nodes+qnodes)*1000/elapsedMs:0,extensionStats:stats,moveOrderingStats:ordering,...(lmr?{lmrStats:lmr}:{}),...(pruning?{pruningStats:pruning}:{}),...(aspiration?{aspirationStats:aspiration}:{})};
}

function makeEmptyAspirationStats():AspirationStats{return {completedIterations:0,aspirationIterations:0,failLow:0,failHigh:0,retries:0,maxRetriesForOneDepth:0,failedAttemptNodes:0,failedAttemptQnodes:0,retryHistogram:[],initialWindowWidthHistogram:{},finalWindowWidthHistogram:{}};}

function makeEmptyPruningStats():PruningStats{const c=()=>({eligibleNodes:0,attempted:0,cutoffs:0,depthHistogram:Array(65).fill(0)});return {reverseFutility:c(),razoring:c(),nullMove:{...c(),nullSearches:0},probCut:{...c(),capturesTried:0,successfulCutoffs:0},lmp:{...c(),breakEvents:0,estimatedMovesSkipped:0}};}
