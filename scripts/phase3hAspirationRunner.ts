...
import {
  PHASE3H_ASPIRATION_START_SUITE,
  PHASE3H_ASPIRATION_V1_FINGERPRINT,
} from './tournamentStartSuite';

const CONFIG = 'config/phase3h-aspiration-v1.json';
type Candidate = Exclude<AspirationProfile, 'current'>;
type Config = {
  ...
  baselinePruningProfile: 'current';
  baselineAspirationProfile: 'current';
};
...
const metrics = (totals: EngineTotals) => ({
  ...
  nps: totals.nps,
  moves: totals.moves,
  aspirationStats: totals.aspirationStats,
});
...
function validate() {
  if (config.suite !== PHASE3H_ASPIRATION_START_SUITE.version ||
      config.seed !== PHASE3H_ASPIRATION_START_SUITE.seed ||
      config.fingerprint !== PHASE3H_ASPIRATION_V1_FINGERPRINT) {
    throw new Error('frozen Phase 3H corpus identity mismatch');
  }
  if (JSON.stringify(config.baselineSearch) !== JSON.stringify(DEFAULT_SEARCH_FEATURES) ||
      ...
      JSON.stringify(config.baselineMoveOrdering) !== JSON.stringify(DEFAULT_MOVE_ORDERING_FEATURES) || config.baselineLmrProfile !== 'current' || config.baselinePruningProfile !== 'current' || config.baselineAspirationProfile !== 'current') {
    throw new Error('explicit Teacher v1 baseline no longer matches production semantics');
  }
}

/** Exported so tests can prove that aspiration measurement collection is
 * explicitly enabled for both sides without running a tournament. */
export function makePhase3hEnginePair(feature: Candidate, search: SearchControl): [EngineConfig, EngineConfig] {
  const instrument = search.mode === 'nodes';
  const baselineMeasurement = { lmrProfile:'current', pruningProfile:'current', aspirationProfile:'current', ...(instrument?{collectAspirationStats:true}:{}) } as const;
  const candidateMeasurement = { lmrProfile:'current', pruningProfile:'current', aspirationProfile:feature, ...(instrument?{collectAspirationStats:true}:{}) } as const;
  return [
    {
      id: 'baseline',
    ...
    {
      id: `profile-${feature}`,
      name: `Teacher v1 aspiration ${feature}`,
      search,
      featureOverrides: config.baselineSearch,
      extensionOverrides: config.baselineExtensions,
      moveOrderingOverrides: config.baselineMoveOrdering,
...
async function run(feature: Candidate, control: SearchControl, offset: number, pairs: number,
  ...
  const [baseline, candidate] = makePhase3hEnginePair(feature, control);
  const result = await runTournament(baseline, candidate, PHASE3H_ASPIRATION_START_SUITE, {
    startOffset: offset,
    startLimit: pairs,
    maxPlies,
...
async function main() {
  ...
  const output = arg('output') ?? `.tmp/phase3h-${stage}`;
  ...
  if (stage === 'screening') {
    ...
    writeFileSync(`${output}/summary.json`, JSON.stringify({ schemaVersion: 'makhos-phase3h-screening-v1',
  ...
  const selectionPath=arg('selection')??'benchmarks/phase3h/screening/summary.json';
  const selection=JSON.parse(readFileSync(selectionPath,'utf8')) as {selected:Candidate|null};
  if(selection.selected!==feature)throw new Error(`--profile must equal frozen screening selection (${selection.selected??'none'})`);
  ...
  writeFileSync(`${output}/summary.json`, JSON.stringify({ schemaVersion: 'makhos-phase3h-confirmation-v1',
