...
import { EXTENSION_CONFIRMATION_START_SUITE, EXTENSION_CONFIRMATION_V1_FINGERPRINT } from './tournamentStartSuite';
...
if (!Number.isInteger(pairs) || pairs < 1 || pairs > EXTENSION_CONFIRMATION_START_SUITE.starts.length)
...
async function run(control: SearchControl, label: string) {
  ...
  const result = await runTournament(baseline, candidate, EXTENSION_CONFIRMATION_START_SUITE,
...
(async () => {
  ...
  writeFileSync(`${output}/summary.json`, JSON.stringify({
    schemaVersion: 'makhos-phase3c-promotion-v1', suite: EXTENSION_CONFIRMATION_START_SUITE.version,
    seed: EXTENSION_CONFIRMATION_START_SUITE.seed, fingerprint: EXTENSION_CONFIRMATION_V1_FINGERPRINT,
