...
import { PHASE3C_FINAL_CONFIRMATION_START_SUITE, PHASE3C_FINAL_CONFIRMATION_V1_FINGERPRINT } from './tournamentStartSuite';
...
if (!Number.isInteger(pairs) || pairs < 1 || pairs > PHASE3C_FINAL_CONFIRMATION_START_SUITE.starts.length)
...
async function run(control: SearchControl, label: string) {
  ...
  const result = await runTournament(baseline, candidate, PHASE3C_FINAL_CONFIRMATION_START_SUITE,
...
(async () => {
  ...
  writeFileSync(`${output}/summary.json`, JSON.stringify({
    schemaVersion: 'makhos-phase3c-promotion-v1', suite: PHASE3C_FINAL_CONFIRMATION_START_SUITE.version,
    seed: PHASE3C_FINAL_CONFIRMATION_START_SUITE.seed, fingerprint: PHASE3C_FINAL_CONFIRMATION_V1_FINGERPRINT,
