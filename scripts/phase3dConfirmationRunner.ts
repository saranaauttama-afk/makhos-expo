...
const startOffset = Number(value('start-offset') ?? 0);
...
if (!Number.isInteger(startOffset) || startOffset < 0 || startOffset > 63) throw new Error('start-offset must select 0..63');
if (!Number.isInteger(pairs) || pairs < 1 || startOffset + pairs > 64) throw new Error('start-offset + pairs must select 1..64 frozen starts');
...
async function run(control: SearchControl, label: string) {
  ...
  const result = await runTournament(baseline, candidate, PHASE3D_CONFIRMATION_START_SUITE,
    { startOffset, startLimit: pairs, maxPlies, allowDescriptiveStatistics: control.mode === 'time' });
...
(async () => {
  ...
  writeFileSync(`${output}/summary.json`, JSON.stringify({ schemaVersion: 'makhos-phase3d-confirmation-v1',
    ...
    startOffset, startNumberRange: [startOffset + 1, startOffset + pairs], pairs, maxPlies, maxDepth: 64,
    nodeBudgets, timeBudgets, results }, null, 2) + '\n');
