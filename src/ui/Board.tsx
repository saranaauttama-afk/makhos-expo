...
export const Board: React.FC<Props> = ({ pos, onTapSquare, fromSquares, selectedFrom, destSquares, lastMove = null }) => {
  ...
  const moveProgress = [0, 0.2, 0.45, 0.7, 0.9, 1][Math.min(moveAnimFrame, 5)];
  const moveLift = [0, 8, 14, 10, 4, 0][Math.min(moveAnimFrame, 5)];
  ...
  const movingSide = (-pos.side) as 1 | -1;
  const movingPiece =
    lastMove
      ? pieces.find(p => p.i === lastMove.to && p.side === movingSide) ?? null
      : null;
  ...
  return (
    <View style={{ width: SIZE.board, height: SIZE.board }}>
      <Svg width={SIZE.board} height={SIZE.board} style={{ position: 'absolute' }}>
        ...
        {pieces.map((p, i) => {
          if (movingPiece && lastMove && moveAnimFrame < 5 && p.i === movingPiece.i && p.side === movingPiece.side) {
            return null;
          }
          const { r, c } = toRC(p.i);
          const x = c * S;
          ...
          const body = S - pad * 2;
          const inset = S * 0.08;
          const shadowFill = p.side === 1 ? PIECE_P1_SHADOW : PIECE_P2_SHADOW;
          const baseFill = p.side === 1 ? COLORS.pieceP1 : COLORS.pieceP2;
        ...
        {movingPiece && lastMove && moveAnimFrame < 5 && (() => {
          const from = toRC(lastMove.from);
          const to = toRC(lastMove.to);
          const c = from.c + (to.c - from.c) * moveProgress;
          const r = from.r + (to.r - from.r) * moveProgress;
          const x = c * S;
          const y = r * S - moveLift;
          const pad = S * 0.16;
          const body = S - pad * 2;
          const inset = S * 0.08;
          const shadowFill = movingPiece.side === 1 ? PIECE_P1_SHADOW : PIECE_P2_SHADOW;
          const baseFill = movingPiece.side === 1 ? COLORS.pieceP1 : COLORS.pieceP2;

          return (
            <G key={`moving-${moveKey}`}>
              <Rect x={x + pad + 2} y={y + pad + 4} width={body} height={body} fill={shadowFill} opacity={0.95} />
              <Rect x={x + pad} y={y + pad} width={body} height={body} fill={baseFill} />
              <Rect x={x + pad + 4} y={y + pad + 4} width={body - 8} height={body - 8} fill="rgba(255,255,255,0.18)" />
        ...
        {movingPiece && lastMove && isPromoteMove && moveAnimFrame < 5 && (() => {
          const to = toRC(lastMove.to);
          const x = to.c * S;
          const y = to.r * S;
